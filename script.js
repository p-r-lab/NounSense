...
const endings = [
    "ung", "heit", "keit", "schaft", "tion", "ur",
    "nis", "chen", "lein", "ment", "um",
    "er", "ling", "ismus", "el", "en", "e"
];
...
function getLogic(word) {
    ...
    // Important change:
    // If there is no useful ending rule, show nothing.
    if (!matchedRule) {
        return "";
    }
...
function showGroup() {
    ...
    document.getElementById("groupWords").innerHTML = selectedWords.map(word => `
        <div class="group-word">
            <strong>${word.article} ${word.noun}</strong>
            <span>${word.meaning || "No meaning available"}</span>
        </div>
    `).join("");
}
